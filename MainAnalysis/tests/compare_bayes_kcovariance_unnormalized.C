...
int quantitate(char const* config, char const* selections, char const* output) {
    ...
    zip([&](auto& fafter, auto const& after) {
        fafter = new TFile(("unfolded/" + set + "/kCovToys/" + after).data(), "read");
    }, fafters, afters);
    ...
    auto p1 = new paper(set + "_" + tag + "_bayes_kcovtoys_unnormalized_comparison_" + label + "_dj", hb);
    ...
    auto p2 = new paper(set + "_" + tag + "_bayes_kcovtoys_unnormalized_comparison_" + label + "_jpt", hb);
