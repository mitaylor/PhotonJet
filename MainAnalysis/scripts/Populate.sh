...
run_pp() {
    ...
    ./bin/populate_original configs/populate/serial/populate_${tag}_bkg.conf configs/analysis/${set}/analysis_pp.conf /data/submit/mitay/populate/arc/${set}/populate_${tag}_raw/populate_${tag}_bkg.root > log/${set}_populate_${tag}_bkg.txt 2>&1 &
}
