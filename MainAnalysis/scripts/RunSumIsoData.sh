...
for i in ${!files[@]}; do
    ...
    nohup ../bin/populate nohup/${tag}_${index[i]}.conf ../configs/analysis/${set}/analysis_aa.conf /data/submit/mitay/populate/${set}/${tag}_${index[i]}.root  > nohup/log/${set}_${tag}_${index[i]}.txt 2>&1 &
done