...
run_pp() {
    ...
    nohup ./bin/tessellate_squeeze_scan configs/tessellate/scan/tessellate_${tag}.conf configs/analysis/${set}/analysis_pp.conf data/preapproval/${set}/tessellate_${tag}.root > log/${set}_tessellate_${tag}.txt 2>&1 &
}
