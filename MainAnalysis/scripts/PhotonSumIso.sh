...
run_aa() {
    ...
    ./bin/photon_isolation_study configs/populate/preapproval-parallel/populate_${tag}_raw.conf configs/analysis/${set}/analysis_aa.conf data/preapproval/${set}/photon_isolation_study_${tag}_raw.root > log/${set}_photon_isolation_study_${tag}_raw.txt 2>&1 &
