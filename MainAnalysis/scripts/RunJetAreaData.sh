...
for i in ${!files[@]}; do
    ...
    nohup ../bin/jet_area nohup/${tag}_${index[i]}.conf ../configs/analysis/${set}/analysis_aa.conf /data/submit/mitay/jet_area/arc/${set}/${tag}_${index[i]}.root  > nohup/log/jet_area_${set}_${tag}_${index[i]}.txt 2>&1 &
done