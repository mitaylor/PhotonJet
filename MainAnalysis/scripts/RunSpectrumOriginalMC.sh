...
for i in ${!files[@]}; do
    ...
    nohup ../bin/photon_pt_spectrum nohup/${tag}_${index[i]}.conf ../configs/analysis/${set}/analysis_aa.conf /data/submit/mitay/spectrum/${set}/${tag}_${index[i]}.root  > nohup/log/${set}_spectrum_${tag}_${index[i]}.txt 2>&1 &
done