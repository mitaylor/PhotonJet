...
for i in ${!files[@]}; do
    ...
    nohup ../bin/populate_original_rho_match nohup/${tag}_${index[i]}.conf ../configs/analysis/${set}/analysis_aa.conf /data/submit/mitay/populate/arc/${set}/${tag}_original_${index[i]}.root  > nohup/log/${set}_${tag}_original_${index[i]}.txt 2>&1 &
done