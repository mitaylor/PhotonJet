...
int compare_before_unfolding(char const* config, char const* selections, const char* output) {
    ...
    hb->alias("eff", "Energy Regression");
    hb->alias("noeff", "No Energy Regression");

    auto p1 = new paper(set + "_accumulate_" + system + "_er_dr_comparison", hb);
    ...
    auto p2 = new paper(set + "_accumulate_" + system + "_er_jtpt_comparison", hb);
