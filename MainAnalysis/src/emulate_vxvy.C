...
int emulate(char const* config, char const* output) {
    ...
    auto ivy = new interval("v_{y} (cm)"s, 100, -0.2, 0.2);
    auto fvy = std::bind(&interval::book<TH1F>, ivy, _1, _2, _3);

    auto ivx = new interval("v_{x} (cm)"s, 100, -0.2, 0.2);
    auto fvx = std::bind(&interval::book<TH1F>, ivx, _1, _2, _3);

    auto vz = new history<TH1F>("vz"s, "", fvz, 3L);
    auto vy = new history<TH1F>("vy"s, "", fvy, 3L);
    auto vx = new history<TH1F>("vx"s, "", fvx, 3L);
    ...
    tdata->Project((*vy)[0]->GetName(), "vy");
    tcomb->Project((*vy)[1]->GetName(), "vy");

    tdata->Project((*vx)[0]->GetName(), "vx");
    tcomb->Project((*vx)[1]->GetName(), "vx");

    (*vz)[0]->Scale(1. / (*vz)[0]->Integral());
    (*vz)[1]->Scale(1. / (*vz)[1]->Integral());

    (*vy)[0]->Scale(1. / (*vy)[0]->Integral());
    (*vy)[1]->Scale(1. / (*vy)[1]->Integral());

    (*vx)[0]->Scale(1. / (*vx)[0]->Integral());
    (*vx)[1]->Scale(1. / (*vx)[1]->Integral());
    ...
    (*vy)[2]->Divide((*vy)[0], (*vy)[1]);
    (*vx)[2]->Divide((*vx)[0], (*vx)[1]);
