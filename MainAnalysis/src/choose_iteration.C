...
int quantitate(char const* config, char const* selections, char const* output) {
    ...
    auto regularization0 = conf->get<std::string>("regularization0");
    auto regularization1 = conf->get<std::string>("regularization1");
    auto regularization2 = conf->get<std::string>("regularization2");
    auto regularization3 = conf->get<std::string>("regularization3");
    ...
    TFile* freg0 = new TFile((base + regularization0).data(), "read");
    TFile* freg1 = new TFile((base + regularization1).data(), "read");
    TFile* freg2 = new TFile((base + regularization2).data(), "read");
    TFile* freg3 = new TFile((base + regularization3).data(), "read");

    auto reg0 = new history<TH1F>(freg0, tag + "_mse"s);
    auto reg1 = new history<TH1F>(freg1, tag + "_mse"s);
    auto reg2 = new history<TH1F>(freg2, tag + "_mse"s);
    auto reg3 = new history<TH1F>(freg3, tag + "_mse"s);
    ...
    /* determine the nominal regularization to use */
    std::vector<int64_t> choice_init(afters.size(), 1);

    for (size_t i = 0; i < afters.size(); ++i) {
        choice_init[i] = (*reg0)[i]->GetMinimumBin();
        if (choice_init[i] == 1) choice_init[i] = 2;
        std::cout << choice_init[i] << std::endl;
    }

    /* determine the smallest regularization to use */
    std::vector<int64_t> choice(afters.size(), 1);

    for (size_t i = 0; i < afters.size(); ++i) {
        auto val1 = (*reg1)[i]->GetMinimumBin();
        auto val2 = (*reg2)[i]->GetMinimumBin();
        auto val3 = (*reg3)[i]->GetMinimumBin();
        auto max = (val1 > val2) ? val1 : val2;
        max = (max > val3) ? max : val3;
        
        if (max > choice_init[i])   choice[i] = max;
        else                        choice[i] = choice_init[i] + 1;
    ...
    /* rename histograms */
    covariance->rename(tag + "_"s + before_label + "_iteration_raw_sub_pjet_u_dr_jpt_sum0_covariance"s);
    unfolded->rename(tag + "_"s + before_label + "_iteration_raw_sub_pjet_u_dr_jpt_sum0_unfolded"s);
    unfolded_fold0->rename(tag + "_"s + before_label + "_iteration_raw_sub_pjet_u_dr_jpt_sum0_unfolded_fold0"s);
    unfolded_fold1->rename(tag + "_"s + before_label + "_iteration_raw_sub_pjet_u_dr_jpt_sum0_unfolded_fold1"s);
    unfolded_fold0_part1->rename(tag + "_"s + before_label + "_iteration_raw_sub_pjet_u_dr_jpt_sum0_unfolded_fold0_part1"s);
    unfolded_fold1_part1->rename(tag + "_"s + before_label + "_iteration_raw_sub_pjet_u_dr_jpt_sum0_unfolded_fold1_part1"s);
    unfolded_fold0_part2->rename(tag + "_"s + before_label + "_iteration_raw_sub_pjet_u_dr_jpt_sum0_unfolded_fold0_part2"s);
    unfolded_fold1_part2->rename(tag + "_"s + before_label + "_iteration_raw_sub_pjet_u_dr_jpt_sum0_unfolded_fold1_part2"s);
