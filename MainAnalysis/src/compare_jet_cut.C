...
int narrate(char const* config, char const* selections, char const* output) {
    ...
    for (int64_t i = 0; i < (int64_t) cut.size(); ++i) { 
        aa_unfolded_fold0[i] = new history<TH1F>(f_aa, "aa_" + label + std::to_string(i) + "_fold0");
        aa_unfolded_fold1[i] = new history<TH1F>(f_aa, "aa_" + label + std::to_string(i) + "_fold1");
        pp_unfolded_fold0[i] = new history<TH1F>(f_pp, "pp_" + label + std::to_string(i) + "_fold0");
