...
int quantitate(char const* config, char const* selections, char const* output) {
    ...
    auto minimum = [&](int64_t index) {
        auto min = "Regularization: k_{reg} = "s + to_text(choice_svd[index-1]) + " and iteration = "s + to_text(choice_bayes[index-1]);
    ...
    for (size_t i = 0; i < filenames.size(); ++i) {
        ...
        p1->stack((*unfolded_bayes_fold0)[i], "data", "Bayes");
    }
    ...
    for (size_t i = 0; i < filenames.size(); ++i) {
        ...
        p2->stack((*unfolded_bayes_fold1)[i], "data", "Bayes");
    }
