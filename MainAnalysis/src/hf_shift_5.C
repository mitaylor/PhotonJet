...
int hf_shift(char const* config, char const* output) {
    ...
    auto ihf = new interval("HF Energy"s, 100, 0, 50000);
    ...
    auto fhnp = [&](int64_t, std::string const& name, std::string const& label) {
        return new TProfile(name.data(), (";Ncoll;HF Energy;"s + label).data(), 100, 1.712, 2100, 0, 50000, "LE"); };
    ...
    for (auto const& file : hp_input) {
        ...
        for (int64_t i = 0; i < nentries; ++i) {
            ...
            for (size_t j = 0; j < hp_pjt->pfEnergy->size(); ++j) {
                ...
                if ((*hp_pjt->pfId)[j] >= 6 && (*hp_pjt->pfPt)[j] >= 5) {
    ...
    for (auto const& file : mb_input) {
        ...
        for (int64_t i = 0; i < nentries; ++i) {
            ...
            for (size_t j = 0; j < mb_pjt->pfEnergy->size(); ++j) {
                if ((*mb_pjt->pfId)[j] >= 6 && (*mb_pjt->pfPt)[j] >= 5) {
    ...
    auto c9 = new paper(tag + "_5_comp_rho_v_ncoll", hb);
    c9->divide(1, -1);
