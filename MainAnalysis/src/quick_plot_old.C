...
int congratulate() {
    ...
    /* open input files */
    auto file = new TFile("data/preapproval/pho_40/gather_hybrid.root", "read"); // change

    /* define kinematics and luminosity */
    auto text_photon_pt = "40 < p_{T}^{#gamma} < 200 GeV"s; // change
    auto text_photon_eta = "|#eta^{#gamma}| < 1.44"s;
    auto text_dphi = "#Delta#phi_{j#gamma} > #frac{7#pi}{8}"s; // change
    ...
    for (int i = 0; i < ncols; ++i) {
        /* get histograms */
        hists[i] = new history<TH1F>(file, "pp_dr_elastic_wake"); // change
    }
    ...
    legend.AddEntry(&graphs_hists[0], "Hybrid pp (old)", "plf"); // change
    ...
    for (int i = 0; i < ncols; i++) {
        ...
        auto text_jet_pt = "30 < p_{T}^{jet} < 120 GeV"s; // change
    ...
    canvas.SaveAs("hybrid_spectra_pp_old.pdf"); // change
