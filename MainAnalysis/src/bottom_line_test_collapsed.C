...
int bottom_line_test(char const* config, char const* selections, char const* output, int iterations) {
    ...
    for (size_t i = 0; i < after_file.size(); ++ i) {
        ...
        std::cout << "smeared: " << ((*step2_smear_fold0)(0,0) + (*step2_smear_fold1)(0,0)) / 224.0 << std::endl;
        ...
        std::cout << "unfolded: " << ((*step2_unfolded_fold0)(0,0) + (*step2_unfolded_fold1)(0,0)) / 90.0 << std::endl << std::endl;
    }
