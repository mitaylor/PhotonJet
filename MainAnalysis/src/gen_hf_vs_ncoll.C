...
int hf_shift(char const* config, char const* output) {
    ...
    auto fmnp = [&](int64_t, std::string const& name, std::string const& label) {
        return new TProfile(name.data(), (";Ncoll;#mult;"s + label).data(), 100, 0, 2100, 0, 400, "LE"); };
    ...
    auto hp_mn_p = new history<TProfile>("hp_mn_p"s, "Pythia+Hydjet", fmnp, 1);
    auto mb_mn_p = new history<TProfile>("mb_mn_p"s, "Hydjet", fmnp, 1);
    ...
    TTreeReaderValue<std::vector<float>> hp_pt(hp_gen, "pt");
    TTreeReaderValue<std::vector<float>> hp_eta(hp_gen, "eta");
    ...
    TTreeReaderValue<std::vector<float>> mb_pt(mb_gen, "pt");
    TTreeReaderValue<std::vector<float>> mb_eta(mb_gen, "eta");
    ...
    for (int64_t i = 0; i < nentries; ++i) {
        ...
        hp_evt.Next(); hp_gen.Next();

        (*hp_hn)[0]->Fill(*hp_hf, *hp_ncoll, *hp_weight);
        (*hp_mn)[0]->Fill(*hp_mult, *hp_ncoll, *hp_weight);

        (*hp_hn_p)[0]->Fill(*hp_ncoll, *hp_hf, *hp_weight);
        (*hp_mn_p)[0]->Fill(*hp_ncoll, *hp_mult, *hp_weight);
    ...
    for (int64_t i = 0; i < nentries; ++i) {
        ...
        mb_evt.Next(); mb_gen.Next();

        (*mb_hn)[0]->Fill(*mb_hf, *mb_ncoll, *mb_weight);
        (*mb_mn)[0]->Fill(*mb_mult, *mb_ncoll, *mb_weight);

        (*mb_hn_p)[0]->Fill(*mb_ncoll, *mb_hf, *mb_weight);
        (*mb_mn_p)[0]->Fill(*mb_ncoll, *mb_mult, *mb_weight);
    ...
    auto diff_mn_p = (TH1*) (*hp_mn_p)[0]->Clone();

    diff_hn_p->SetNameTitle("diff_hn_p", ";;Orange - Purple");
    diff_mn_p->SetNameTitle("diff_mn_p", ";;Orange - Purple");

    if (!(diff_hn_p->GetSumw2N() > 0)) diff_hn_p->Sumw2(true);
    if (!(diff_mn_p->GetSumw2N() > 0)) diff_mn_p->Sumw2(true);

    diff_hn_p->Add((*mb_hn_p)[0], -1);
    diff_mn_p->Add((*mb_mn_p)[0], -1);

    diff_hn_p->SetMaximum(300);
    diff_hn_p->SetMinimum(-300);
    diff_mn_p->SetMaximum(10);
    diff_mn_p->SetMinimum(-10);
    ...
    auto c9 = new paper("forest_comp_mult_v_ncoll", hb);
    c9->divide(1, -1);
    apply_style(c9, cms, system_tag);
    c9->add((*mb_mn_p)[0], "Hydjet");
    c9->stack((*hp_mn_p)[0], "Pythia+Hydjet");
    c9->add(diff_mn_p);
