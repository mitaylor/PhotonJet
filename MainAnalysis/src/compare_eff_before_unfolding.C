...
int compare_before_unfolding(char const* config, const char* output) {
    ...
    h_eff_dr->rename("aa_eff_dr");
    auto h_noeff_dr = new history<TH1F>(f_noeff, "aa_nominal_s_pure_raw_sub_pjet_f_dr_sum0");
    h_noeff_dr->rename("aa_noeff_dr");
    auto h_eff_jtpt = new history<TH1F>(f_eff, "aa_nominal_s_pure_raw_sub_pjet_f_jpt_sum0");
    h_eff_jtpt->rename("aa_eff_jtpt");
    auto h_noeff_jtpt = new history<TH1F>(f_noeff, "aa_nominal_s_pure_raw_sub_pjet_f_jpt_sum0");
    h_noeff_jtpt->rename("aa_noeff_jtpt");
    ...
    hb->alias("eff", "Efficiency Correction Applied");
    hb->alias("noeff", "No Correction");
