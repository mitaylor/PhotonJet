...
int quantitate(char const* config, char const* selections, char const* output) {
    ...
    std::vector<history<TMatrixT<double>>*> covariance;
    ...
    covariance.push_back(new history<TMatrixT<double>>*>("covariance_regularization"s + regularization, ""s, null<TMatrixT<double>>, (int64_t) afters.size()));

    /* extract histograms */
    for (size_t j = 0; j < afters.size(); ++j) {
        std::string unfold_name = "HUnfoldedBayes" + regularization;
    ...
    covariance[0]->apply([&](TH1* h) { p4->adjust(h, "colz", ""); });
