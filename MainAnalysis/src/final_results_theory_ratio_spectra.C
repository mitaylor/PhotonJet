...
int congratulate(char const* config, char const* selections, char const* output) {
    ...
    if (system == 4)    legend_y_min = (subsets) ? 0.48 : 0.73;
    if (system == 4)    legend_y_max = (subsets) ? 0.83 : 0.80;
    ...
    if (system == 4 && !subsets)    latex.DrawLatex(0.06, legend_y_max + 0.02, "HYBRID");
    ...
    if (system == 2 && !subsets)    latex.SetTextAlign(11);
    if (system == 2 && !subsets)    latex.DrawLatex(0.05, 0.80, (text_photon_pt + ", " + text_photon_eta).c_str());
    if (system == 2 && !subsets)    latex.DrawLatex(0.05, 0.72, (text_dphi + ", " + text_jet_eta + ", " + text_jet_alg).c_str());
