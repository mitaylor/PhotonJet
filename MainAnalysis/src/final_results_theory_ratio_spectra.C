...
int congratulate(char const* config, char const* selections, char const* output) {
    ...
    for (int i = 0; i < ncols; i++) {
        ...
        if (system == 2)    (*hists_ratio_pyquen_no_wide[i])[0]->Draw("same c e3");
        if (system == 2)    (*hists_ratio_pyquen[i])[0]->Draw("same c e3");
        if (system == 2)    (*hists_ratio_jewel[i])[0]->Draw("same c e3");
        if (system == 2)    (*hists_ratio_jewel_no_recoil[i])[0]->Draw("same lf2");
