...
int congratulate(char const* config, char const* selections, char const* output) {
    ...
    if (system == 4)    legend_x_min = (subsets) ? 0.05 : 0.53;
    if (system == 4)    legend_x_max = (subsets) ? 0.35 : 0.83;
    ...
    if (system == 4 && !subsets)    legend_x_min = legend_x_min - 0.5;
    if (system == 4 && !subsets)    legend_x_max = legend_x_max - 0.5;
