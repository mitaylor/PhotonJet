...
int bottom_line_test(char const* config, char const* selections, char const* output) {
    ...
    zip([&](auto& fafter, auto const& after) {
        fafter = new TFile(("unfolded/Errors/"s + set + "/Bayes/Original/kErrors/"s + after).data(), "read");
    }, fafter, after_file);
    ...
    for (int64_t i = 0; i < size; ++i) {
        (*data_before)[i]->Divide((*reco_eff)[i]);
    ...
    for (int i = 0; i < size; ++i) {
        ...
        for (int j = 0; j < (*data_before_fold0)[i]->GetNbinsX(); ++j) {
            ...
            auto err = (*data_before_fold0)[i]->GetBinError(j+1);
            std::cout << diff << " " << diff/err/err << " " << diff * diff / (err * err) << " " << err * err << " " << 1/err/err << std::endl;
            chi2_smear_fold0 += diff * diff / (err * err);
        }
        std::cout << chi2_smear_fold0 << std::endl;
    ...
    for (size_t k = 0; k < iterations.size(); ++k) {
        ...
        for (int i = 0; i < size; ++i) {
            std::string unfold_name = "Test0HUnfoldedBayes" + std::to_string(iterations[k]);
            std::string matrix_name = "HCovarianceDist" + std::to_string(iterations[k]);
            std::string matrix_fold0_name = "HCovarianceDist" + std::to_string(iterations[k]) + "Fold0";
            std::string matrix_fold1_name = "HCovarianceDist" + std::to_string(iterations[k]) + "Fold1";

            auto HUnfolded = (TH1F*) fafter[i]->Get(unfold_name.data());

            auto MUnfolded = (TH2F*) fafter[i]->Get(matrix_name.data());
            auto MUnfoldedFold0 = (TH2F*) fafter[i]->Get(matrix_fold0_name.data());
            ...
            for (int j = 0; j < (*data_after)[i]->GetNbinsX(); ++j) {
                for (int k = 0; k < (*data_after)[i]->GetNbinsX(); ++k) {
                    covariance_after_elements[j * (*data_after)[i]->GetNbinsX() + k] = MUnfolded->GetBinContent(j + 1, k + 1);
                }
            }

            for (int j = 0; j < (*data_after_fold0)[i]->GetNbinsX(); ++j) {
                for (int k = 0; k < (*data_after_fold0)[i]->GetNbinsX(); ++k) {
                    covariance_after_fold0_elements[j * (*data_after_fold0)[i]->GetNbinsX() + k] = MUnfoldedFold0->GetBinContent(j + 1, k + 1);
                }
            }

            for (int j = 0; j < (*data_after_fold1)[i]->GetNbinsX(); ++j) {
                for (int k = 0; k < (*data_after_fold1)[i]->GetNbinsX(); ++k) {
                    covariance_after_fold1_elements[j * (*data_after_fold1)[i]->GetNbinsX() + k] = MUnfoldedFold1->GetBinContent(j + 1, k + 1);
                }
            }

            covariance_matrix_after[i] = new TMatrixT<double>((*data_after)[i]->GetNbinsX(), (*data_after)[i]->GetNbinsX(), &covariance_after_elements[0]);
    ...
    for (int i = 0; i < size; ++i) {
        (*chi2_before_dj)[i]->SetMaximum((*chi2_before_dj)[i]->GetMaximum()*2);
    ...
    for (int64_t i = 0; i < size; ++i) {
        (*chi2_before_jpt)[i]->SetMaximum((*chi2_before_jpt)[i]->GetMaximum()*2);
