...
int populate(char const* config, char const* selections, char const* output) {
    ...
    std::vector<float> vz_bins = {-20, -5, -2, 0, 2, 5, 20};
    ...
    auto mhfvz = new multival(hf_bins, vz_bins);
    ...
    for (int64_t i = 0; i < mentries; ++i){
        ...
        hf_map[hfvz_x].push_back(jet_vector);
    }
        ...
        for (int64_t i = 0; i < nentries; ++i) {
            ...
            if (mix > 0) {
                ...
                for (int64_t k = 0; k < mix; k++) {
                    std::vector<std::map<std::string, float>> jet_vector = hf_map[hfvz_x][map_x];
                    ...
                    map_x++;
                    if (map_x >= hf_map[hfvz_x].size()) { map_x = 0; }
                }
            }
        }
