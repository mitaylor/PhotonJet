...
void fill_axes(pjtree* pjt, multival* mpthf, multival* mpthfeta, interval *ieta,
               ...
               memory<TH1F>* pjet_f_jpt, memory<TH1F>* pjet_f_dphi, memory<TH1F>* pjet_f_dr_eta
            ) {
    ...
    for (int64_t j = 0; j < pjt->nref; ++j) {
        ...
        if (mpthfeta_x > -1 && mpthfeta_x < mpthfeta->size() && jet_dr < jet_dr_max) {
            (*pjet_f_dr_eta)[mpthfeta_x]->Fill(jet_dr, weight);
...
int populate(char const* config, char const* selections, char const* output) {
    ...
    auto pjet_f_dr_eta = new memory<TH1F>("pjet_f_dr_eta"s, 
    ...
    auto mix_pjet_f_dphi = new memory<TH1F>("mix_pjet_f_dphi"s,
        "1/N^{#gamma} dN/d#Delta#phi", fdphi, mpthf);
    auto mix_pjet_f_dr_eta = new memory<TH1F>("mix_pjet_f_dr_eta"s, 
        "1/N^{#gamma} dN/d#deltaj", fdr, mpthfeta);
    ...
    for (auto const& file : input) {
        ...
        for (int64_t i = 0, m = 0; i < nentries; ++i) {
            ...
            /* fill histograms */
            fill_axes(pjt, mpthf, mpthfeta, ijpt, pt_x, hf_x, weight,
            ...
            if (mix > 0) {
                ...
                for (int64_t k = 0; k < mix; m++) {
                    ...
                    fill_axes(pjtm, mpthf, mpthfeta, ijpt, pt_x, hf_x, weight,
                        photon_phi, exclude, jet_eta_abs, jet_dr_max, 
                        jet_pt_min, jet_pt_max, dphi_min_numerator, dphi_min_denominator,
    ...
    /* scale histograms */
    if (mix > 0) {
        scale(1. / mix, mix_pjet_f_dr, mix_pjet_f_jpt, mix_pjet_f_dphi, mix_pjet_f_dr_eta);
    }
    ...
    auto sub_pjet_f_dr_eta = new memory<TH1F>(*pjet_f_dr_eta, "sub");

    *sub_pjet_f_dr -= *mix_pjet_f_dr;
    *sub_pjet_f_jpt -= *mix_pjet_f_jpt;
    *sub_pjet_f_dphi -= *mix_pjet_f_dphi;
    *sub_pjet_f_dr_eta -= *mix_pjet_f_dr_eta;
