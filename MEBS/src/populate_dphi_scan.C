...
void fill_axes(pjtree* pjt, multival* mpthf, multival* mpthfdphi, interval *idphi,
    ...
    for (int64_t j = 0; j < pjt->nref; ++j) {
        ...
        if (mpthfdphi_x > -1 && mpthfdphi_x < mpthfdphi->size() && jet_dr < jet_dr_max) {
            (*pjet_f_dr_dphi)[mpthfdphi_x]->Fill(jet_dr, weight);
...
int populate(char const* config, char const* selections, char const* output) {
    ...
    auto pjet_f_dr_dphi = new memory<TH1F>("pjet_f_dr_dphi"s, 
    ...
    auto mix_pjet_f_dphi = new memory<TH1F>("mix_pjet_f_dphi"s,
        "1/N^{#gamma} dN/d#Delta#phi", fdphi, mpthf);
    auto mix_pjet_f_dr_dphi = new memory<TH1F>("mix_pjet_f_dr_dphi"s, 
        "1/N^{#gamma} dN/d#deltaj", fdr, mpthfdphi);
    ...
    /* scale histograms */
    if (mix > 0) {
        scale(1. / mix, mix_pjet_f_dr, mix_pjet_f_jpt, mix_pjet_f_dphi, mix_pjet_f_dr_dphi);
    }
    ...
    auto sub_pjet_f_dr_dphi = new memory<TH1F>(*pjet_f_dr_dphi, "sub");

    *sub_pjet_f_dr -= *mix_pjet_f_dr;
    *sub_pjet_f_jpt -= *mix_pjet_f_jpt;
    *sub_pjet_f_dphi -= *mix_pjet_f_dphi;
    *sub_pjet_f_dr_dphi -= *mix_pjet_f_dr_dphi;
