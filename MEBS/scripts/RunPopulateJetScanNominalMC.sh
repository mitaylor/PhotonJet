...
for i in ${!files[@]}; do
    ...
    nohup ../bin/populate_jpt_scan nohup/${tag}_${index[i]}.conf ../configs/analysis/${set}/analysis_aa.conf /data/submit/mitay/mebs/${set}/${tag}_original_${index[i]}.root  > nohup/log/${set}_${tag}_original_${index[i]}.txt 2>&1 &
done