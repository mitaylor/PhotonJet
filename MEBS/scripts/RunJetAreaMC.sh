...
for i in ${!files[@]}; do
    ...
    nohup ../bin/jet_area nohup/${tag}_${index[i]}.conf ../configs/analysis/${set}/analysis_aa.conf /data/submit/mitay/mebs/jet_area/${set}/${tag}_fast_${index[i]}.root  > nohup/log/${set}_${tag}_fast_${index[i]}.txt 2>&1 &
done