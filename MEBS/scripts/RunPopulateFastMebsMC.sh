...
for i in ${!files[@]}; do
    ...
    nohup ../bin/populate_fast_mebs nohup/${tag}_${index[i]}.conf ../configs/analysis/${set}/analysis_aa.conf /data/submit/mitay/mebs/${set}/${tag}_fast_mebs_${index[i]}.root  > nohup/log/${set}_${tag}_fast_mebs_${index[i]}.txt 2>&1 &
done