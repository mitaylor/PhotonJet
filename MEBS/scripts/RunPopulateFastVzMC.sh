...
for i in ${!files[@]}; do
    ...
    nohup ../bin/populate_fast_vz nohup/${tag}_${index[i]}.conf ../configs/analysis/${set}/analysis_aa.conf /data/submit/mitay/mebs/${set}/${tag}_fast_vz_${index[i]}.root  > nohup/log/${set}_${tag}_fast_vz_${index[i]}.txt 2>&1 &
done