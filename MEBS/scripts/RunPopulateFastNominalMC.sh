...
for i in ${!files[@]}; do
    ...
    nohup ../bin/populate_fast nohup/${tag}_${index[i]}.conf ../configs/analysis/${set}/analysis_aa.conf /data/submit/mitay/mebs/${set}/${tag}_fast_${index[i]}.root  > nohup/log/${set}_${tag}_fast_${index[i]}.txt 2>&1 &
done